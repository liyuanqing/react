...
class communityManagerAdd extends React.Component {
    ...
    handleSubmit(){
        ...
        this.props.form.validateFields((err, values) => {
            ...
            if(values.logo_url!=undefined&&values.logo_url.length!=0){
            logo_urls=values.logo_url[0].response.data.imgPath+values.logo_url[0].response.data.message
            }
            if(values.img_list!=undefined&&values.img_list.length!=0)
 ...
 <Form>
        ...
        <Modal visible={this.state.previewVisible} footer={null} onCancel={this.handleCancel.bind(this)}>
          ...
          <FormItem label="是否显示" {...Layout}>
            {getFieldDecorator('status',{rules:[{required:true,message:'请选择是否显示'}],initialValue:"1"})(
              <RadioGroup>
