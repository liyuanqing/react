...
class communityManagerEdit extends React.Component {
    ...
    handleSubmit(){
        ...
        this.props.form.validateFields((err, values) => {
            ...
            let {city_id,name,group,address,pro_company_id,phone,service_list,logo_url,img_list,status,comm_type,house_id} = values;
            var city_ids='';
            var district_code='';
            var province_code='';
            var logo_urls='';
            var img_lists=[];
            province_code=values.city_id[0];
            city_ids=values.city_id[1];
            district_code=values.city_id[2];
            // console.log(values.logo_url[0].response.data.imgPath+values.logo_url[0].response.data.message)
          if(values.logo_url.length>0){  //判断已上传图片
 ...
 <Form>
        ...
        <FormItem label="小区类型" {...Layout}>
              {getFieldDecorator('comm_type', {
                  rules: [
                      {required: true,message: '请选择小区类型'},
                  ],
                  initialValue:info.comm_type
              })(
                <Select placeholder="请选择小区类型" onChange={this.handleChange.bind(this)}>
                   <Option key={1} value="1">物业</Option>
                   <Option key={2} value="2">新房</Option>
                </Select>
              )}
          </FormItem>
          {show==true?
          <FormItem label="楼盘id" {...Layout}>
              {getFieldDecorator('house_id', {
                  rules: [
                      {required: true,message: '请输入正确id!',pattern: /^[0-9,]+$/,type: "string"},
                  ],
                  initialValue:info.house_id
              })(
              <Input maxLength={11} placeholder="请输入楼盘id(例：11,33,44)" />
              )}
          </FormItem>
          :""}
            <FormItem label="关联企业" {...Layout}>
            {getFieldDecorator('pro_company_id', {
                rules: [
                    {required: true, message: '请选择关联企业!'},
                ],
                initialValue:info.pro_company_id
            })(
            <Select placeholder="请选择企业"  notFoundContent="没有数据">
            ...
            {getFieldDecorator('phone', {
                rules: [
                    {type: "string",required: true, message: '请输入物业电话!'},
                ],
        ...
        <Modal visible={this.state.previewVisible} footer={null} onCancel={this.handleCancel.bind(this)}>
          ...
          <FormItem label="是否显示" {...Layout}>
            {getFieldDecorator('status',{rules:[{required:true,message:'请选择是否显示'}],initialValue:info.status})(
              <RadioGroup>
