...
export function getUrl(){
  var location = window.location,
    ...
    if(hostname=='localhost'){   //本地启动接口地址
      return 'http://192.168.1.201:1111'
    }else if(hostname=='wuyeyy.zje.com'){  //内网测试接口地址
      return 'http://116.62.92.115:72'
    }else if(hostname=='hd-wuyeyy.zje.com'){  //线上接口地址
      return 'http://116.62.92.115:71'
    }
}
var tools = {
  /*
  ...
  downLoad(url) {
    console.log(11);
    var html="<a  download='e' href="+url+"  id='btn' style='display:none'></a>"
  ...
  getToken() {
    ...
    //   return $token;
    // }
    $token = document.cookie.split("ztoken")[1].split("=")[1].split(";")[0]
    return $token || '';
  },
  isToken(code){
    if (code && code == 50002) {
...
export const getCookie = tools.getCookie.bind(tools);
export const downLoad= tools.downLoad.bind(tools);
